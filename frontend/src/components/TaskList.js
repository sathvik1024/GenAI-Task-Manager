...
const TaskList = ({ refreshTrigger, onTaskUpdate }) => {
  ...
  const fetchTasks = async () => {
    try {
      ...
      console.log('TaskList: Number of tasks received:', response.tasks ? response.tasks.length : 0);
  ...
  const handlePrioritizeTasks = async () => {
    try {
      ...
      const prioritizedTasks = response.prioritized_tasks;
  ...
  const getFilteredTasksCount = () => {
    const activeFilters = Object.values(filters).filter(value => value.trim() !== '');
